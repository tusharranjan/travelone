...
jQuery(function($) {
	...
	"use strict";
		...
		*/
		...
		$('#enq_nwlist,.top_serach').on('click',function(){
			$('#frmDefaultClientEnquiry').fadeIn();
		})

		$('.pop_advsearch').on('click',function(event){
			if(event.target == this){
				$('#frmDefaultClientEnquiry').fadeOut();
			}

		})
		$('.close-icons').on('click',function(event){
			$('#frmDefaultClientEnquiry').fadeOut();
		})
...
$(window).scroll(function(){
	var scr_postion = $(window).scrollTop();
	console.log(scr_postion);
	if(scr_postion>=40){
